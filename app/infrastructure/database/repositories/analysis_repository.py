...
class AnalysisRepository(IAnalysisRepository):
    ...
    async def get_tourists_for_random_period(self) -> dict:
        ...
        tourists_by_random_period = {"Количество посетителей за рандомный промежуток": []}
        tourists_by_random_period["Количество посетителей за рандомный промежуток"].append({
    ...
    async def get_typical_tourist_profile(self) -> dict:
        db_result = await self.session.execute(select(
