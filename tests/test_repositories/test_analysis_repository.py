...
async def test_get_tourists_for_random_period(async_session, test_records):
    ...
    assert response["Количество посетителей за рандомный промежуток"][0]['Количество человек:'] == 10
...
async def test_get_tourists_by_country(async_session, test_records):
    ...
    assert response["Территориальное распределение по странам"][0]['Количество посетителей'] == 1
...
async def test_get_tourists_by_region(async_session, test_records):
    ...
    assert response["Территориальное распределение по регионам"][0]['Количество посетителей'] == 1
...
async def test_get_typical_tourist_profile(async_session, test_records):
    ...
    assert response['Профиль среднестатестического туриста']['Количество дней в городе'] == 1
...
async def test_get_most_valuable_segments(async_session, test_records):
    ...
    assert response['Популярные категории туристов'][0]['Среднее количество дней в городе'] == 1